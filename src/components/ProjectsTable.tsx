...
export default function ProjectsTable({ categories }: { categories: CategoryData[] }) {
  ...
  const sortedProjects = allProjects.sort((a, b) => {
    const catOrder =
      categories.findIndex((c) => c.category === a.category) -
  ...
  const getCategoryRowspan = (index: number) => {
    ...
    let rowspan = 1;
    for (let i = index + 1; i < sortedProjects.length; i += 1) {
      if (sortedProjects[i].category === currentCategory) rowspan += 1;
      else break;
    }
    return rowspan;
  };
        ...
        <table className="projects-table">
          ...
          <tbody>
            {sortedProjects.map((project, idx) => {
              ...
              return (
                <tr key={idx} className={`category-${catClass}`}>
                  {showCat && (
